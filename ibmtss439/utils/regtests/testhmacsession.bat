...
REM		$Id: testhmacsession.sh 321 2015-06-04 19:14:39Z kgoldman $	#
REM										#
...
create -hp 80000000 -st -kt f -kt p -pwdp pps -pwdk sto -se0 02000000 1 > run.out
IF %ERRORLEVEL% NEQ 0 (
  exit /B 1
  )

echo "Create a storage key under the primary key - continue false"
create -hp 80000000 -st -kt f -kt p -pwdp pps -pwdk sto -se0 02000000 0 > run.out
IF %ERRORLEVEL% NEQ 0 (
  exit /B 1
  )
