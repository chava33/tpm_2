...
REM #		$Id: testpolicy.sh 348 2015-06-22 20:36:45Z kgoldman $		#
REM #										#
...
REM # create privkey.pem
REM # > openssl genrsa -out privkey.pem -aes256 -passout pass:rrrr 2048
REM # extract the public key
REM # > openssl pkey -inform pem -outform pem -in privkey.pem -passin pass:rrrr -pubout -out pubkey.pem 
REM # sign a test message msg.bin
REM # > openssl dgst -sha1 -sign privkey.pem -passin pass:rrrr -out pssig.bin msg.bin
...
echo "Policy ticket"
policyticket -ha 03000000 -to to.bin -na h80000001.bin -tk tkt.bin > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
echo "Policy ticket"
policyticket -ha 03000000 -to to.bin -hi p -tk tkt.bin > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
loadexternal -hi p -halg sha1 -nalg sha1 -ipem policies/pubkey.pem > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
policyauthorize -ha 03000000 -appr policyapproved.bin -skn h80000001.bin -tk tkt.bin > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
setprimarypolicy -hi p > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )

echo "Platform policy empty, bad password"
setprimarypolicy -hi p -pwda ppp > run.out
IF %ERRORLEVEL% EQU 0 (
  exit /B 1
    )
...
setprimarypolicy -hi p -pwda ppp -halg sha256 -pol policies/policysecretp.bin > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
hierarchychangeauth -hi p -se0 03000000 0 > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
nvdefinespace -hi p -nalg sha1 -ha 01000000 -pwdn nnn -ty b > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
nvdefinespace -hi p -nalg sha1 -ha 01000001 -pwdn nnn -sz 2 -ty o -pol policies/policynvnv.bin > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
echo "Define an ordinary index, authorized index, policyNV"
nvdefinespace -hi p -nalg sha1 -ha 01000000 -pwdn nnn -sz 2 -ty o -pol policies/policywrittenset.bin > run.out  
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
setprimarypolicy -hi p -pol policies/policycphash.bin -halg sha1 > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )

echo "Clockrate adjust using wrong password - should fail"
clockrateadjust -hi p -pwdp ppp -adj 0  > run.out 
IF %ERRORLEVEL% EQU 0 (
  exit /B 1
    )
...
clockrateadjust -hi p -pwdp ppp -adj 0 -se0 03000000 1 > run.out
IF %ERRORLEVEL% EQU 0 (
  exit /B 1
    )
...
clockrateadjust -hi p -pwdp ppp -adj 1 -se0 03000000 1 > run.out 
IF %ERRORLEVEL% EQU 0 (
  exit /B 1
    )
...
setprimarypolicy -hi p -pol policies/policycountertimer.bin -halg sha1 > run.out
    IF %ERRORLEVEL% NEQ 0 (
       exit /B 1
       )
...
echo "Clockrate adjust, policy not satisfied - should fail"
clockrateadjust -hi p -adj 0 -se0 03000000 1 > run.out
IF %ERRORLEVEL% EQU 0 (
  exit /B 1
    )
